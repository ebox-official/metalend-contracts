...
/*
 * On deployment a new Uniswap pool will be created for the pair MUSD/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of MLEND token,
 ...
 * in order to avoid getting MLEND tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either MLEND token contract is deployed, and therefore MLEND tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(mlendToken.getLpRewardsEntitlement(), _duration);
    ...
    function claimReward() public override {
        ...
        mlendToken.transfer(msg.sender, reward);
...
//        assert(_reward > 0);
        assert(_reward == mlendToken.balanceOf(address(this)));
