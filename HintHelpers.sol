...
contract HintHelpers is LiquityBase, CheckContract {
        ...
        )
    {
        ...
        uint remainingMUSD = _MUSDamount;
        ...
        while (currentTroveuser != address(0) && remainingMUSD > 0 && _maxIterations-- > 0) {
            uint netMUSDDebt = _getNetDebt(troveManager.getTroveDebt(currentTroveuser))
                .add(troveManager.getPendingMUSDDebtReward(currentTroveuser));

            if (netMUSDDebt > remainingMUSD) {
                if (netMUSDDebt > MIN_NET_DEBT) {
                    uint maxRedeemableMUSD = LiquityMath._min(remainingMUSD, netMUSDDebt.sub(MIN_NET_DEBT));

                    uint DFI = troveManager.getTroveColl(currentTroveuser)
                        .add(troveManager.getPendingETHReward(currentTroveuser));

                    uint newColl = DFI.sub(maxRedeemableMUSD.mul(DECIMAL_PRECISION).div(_price));
                    uint newDebt = netMUSDDebt.sub(maxRedeemableMUSD);

                    uint compositeDebt = _getCompositeDebt(newDebt);
                    partialRedemptionHintNICR = LiquityMath._computeNominalCR(newColl, compositeDebt);

                    remainingMUSD = remainingMUSD.sub(maxRedeemableMUSD);
                }
                break;
            } else {
                remainingMUSD = remainingMUSD.sub(netMUSDDebt);
            }

            currentTroveuser = sortedTrovesCached.getPrev(currentTroveuser);
        }

        truncatedMUSDamount = _MUSDamount.sub(remainingMUSD);
    }
